...
public:
    ...
    void varaa_huone() {
        ...
        std::cout << "Hotellissa nukuttujen ˆiden hinta on " << hinta << " Ä.\n\n";
    }
};
...
int valitseHuone(std::vector<Hotellihuone>& hotellihuoneet, int huonekoko) // Viitataan vektoriin hotellihuoneet, jotta siihen voi tehd‰ muutoksia.
{
    ...
    while (true)
    {
        ...
        if (huonekoko == 1)
        {
            std::string kysymys{ "Ole hyv‰ ja valitse huonenumero (1-" + std::to_string(hotellihuoneet.size() / 2 - 1) + ")" };
            ...
            {
                std::cout << "Huonenumeron pit‰‰ olla v‰lilt‰ 1-" << hotellihuoneet.size() / 2 - 1 << "!\n\n";
                continue;
            }
        }
        else
        {
            std::string kysymys{    "Ole hyv‰ ja valitse huonenumero (" + std::to_string(hotellihuoneet.size() / 2)
                                    + "-" + std::to_string(hotellihuoneet.size()) + ")" };
            valinta = kysySyote(kysymys, "0123456789", false);
            huonenumero = std::stoi(valinta);
            if (huonenumero < hotellihuoneet.size() / 2 || huonenumero > hotellihuoneet.size())
            {
                std::cout << "Huonenumeron pit‰‰ olla v‰lilt‰ " << hotellihuoneet.size() / 2 << "-" << hotellihuoneet.size() << "!\n\n";
                continue;
            }
        }
